...
const DEFAULTS = {
  name:'', category:'Books', subcategory:'', importance:3,
  frequencyDays:14, notes:'', author:'', imageUrl:'', isbn:'', source:'manual'
}
...
export default function ActivityForm({ item, onClose }) {
  ...
  // Initialize form — if editing use item values, otherwise use defaults
  const [f, setF] = useState(() => item ? { ...DEFAULTS, ...item } : { ...DEFAULTS })

  const set = (k, v) => setF(p => ({ ...p, [k]: v }))

  // Called when scanner returns a result — merge into form
    ...
    <>
      ...
      >
        <div style={{ background:'#fff', borderRadius:'22px 22px 0 0', padding:'24px 20px 40px', width:'100%', maxHeight:'92vh', overflowY:'auto', boxSizing:'border-box' }}>
          ...
          {/* Cover image preview (from barcode scan) */}
          {f.imageUrl ? (
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16, padding:12, background:'#f8f7ff', borderRadius:12 }}>
              <img src={f.imageUrl} alt="" style={{ width:50, height:68, objectFit:'cover', borderRadius:6 }} />
              <div>
                <div style={{ fontSize:12, color:'#667eea', fontWeight:600 }}>Cover found ✓</div>
                <button onClick={() => set('imageUrl', '')}
                  style={{ background:'none', border:'none', color:'#aaa', fontSize:11, cursor:'pointer', padding:0, marginTop:2 }}>
                  Remove image
                </button>
              </div>
            </div>
          ...
          <input
            ...
            style={{ ...inp, borderColor: f.name.trim() ? '#e2e8f0' : '#fca5a5' }}
          ...
          <input
            ...
            style={inp}
            placeholder="e.g. J.K. Rowling, LEGO, Hasbro"
          />
          ...
          <input
            ...
            style={inp}
            placeholder="e.g. Science, Puzzle, Lego Technic"
          />
          ...
          <div style={{ display:'flex', gap:6, marginBottom:16 }}>
            {[1,2,3,4,5].map(n => (
              <button key={n} onClick={() => set('importance', n)}
                style={{ background:'none', border:'none', cursor:'pointer', fontSize:28, opacity: f.importance >= n ? 1 : .2, padding:'2px 4px' }}>
                ⭐
              </button>
            ))}
          </div>
          ...
          {/* Frequency */}
          <label style={lbl}>How often? (every N days)</label>
          <div style={{ display:'flex', gap:10, alignItems:'center', marginBottom:16 }}>
            <input
              type="number" value={f.frequencyDays} min={1}
              onChange={e => set('frequencyDays', Math.max(1, parseInt(e.target.value) || 1))}
              style={{ ...inp, marginBottom:0, width:100 }}
            />
            <span style={{ color:'#888', fontSize:13 }}>
              {f.frequencyDays <= 2 ? 'Daily' : f.frequencyDays <= 7 ? 'Weekly' : f.frequencyDays <= 14 ? 'Biweekly' : f.frequencyDays <= 31 ? 'Monthly' : 'Rarely'}
            </span>
          </div>
          ...
          <div style={{ display:'flex', gap:10, marginTop:4 }}>
            ...
            <button onClick={handleSave} disabled={!f.name.trim() || saving}
              style={{ flex:2, background: f.name.trim() ? '#667eea' : '#c4b5fd', border:'none', borderRadius:14, padding:14, color:'#fff', fontWeight:700, fontSize:15, cursor: f.name.trim() ? 'pointer' : 'default' }}>
